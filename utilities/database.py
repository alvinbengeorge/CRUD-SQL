...
class Database:
    ...
    def show_all(self) -> List[tuple]:
        ...
        for i in range(len(result)):
            result[i] = self.convert_to_dict(result[i])
        return result
    ...
    def update(self, id: int, new_data: LogData) -> None:
        with self.connection.cursor() as cursor:
            cursor.execute("UPDATE log SET user = %s, time = %s, description = %s WHERE id = %s", (new_data.user, new_data.time, new_data.description, id))
            self.connection.commit()
    ...
    def search(self, query: str) -> List[tuple]:
        with self.connection.cursor() as cursor:
            ...
            return self.convert_to_dict(cursor.fetchall())
